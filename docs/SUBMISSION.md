...
Khi nhận ground truth ba tier trong giờ cuối, [làm theo hướng dẫn riêng để chạy `score.py` và `scorecard.py --group tiers`](SELF_SCORING.md). Script này **có tính metric khi có reference**, khác với lệnh `inspect_submissions.py` ở trên. Chỉ gói reference chính thức mới được dùng; không đưa đáp án lên fork hay Colab công khai. Nếu máy không chạy được, báo coach trong buổi lab, vẫn giữ ZIP/report và nộp bài cốt lõi.
