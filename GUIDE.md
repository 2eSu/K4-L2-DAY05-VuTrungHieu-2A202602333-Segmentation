...
Theo thứ tự: đúng ảnh → đúng loại segmentation → đúng tên lớp → đủ số object/vùng → biên → Save → ZIP đúng format. Một lỗi thực tế và cách sửa phải xuất hiện trong `REPORT.md`. Trên fork, bật **Actions** một lần; khi upload ZIP vào `submissions/` và Commit, **Day 5 self-check** tự kiểm cấu trúc, **không tính điểm và không tải đáp án**. Sau khi người phụ trách phát reference ba tier, bạn có thể [chạy scorer trên máy](docs/SELF_SCORING.md) để tìm lỗi; [notebook Colab](notebooks/README.md) vẫn là tùy chọn. Bảng mini-hackathon nếu mở dùng bản commit đã khóa và tách khỏi chấm điểm chính thức.
